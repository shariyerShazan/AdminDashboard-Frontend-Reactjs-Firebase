...
import { useAuthenticateMutation } from "@/redux/features/auth/firebase.auth.api";
...
const LoginPage = () => {
  ...
  const handleGoogleLogin = async () => {
    ...
    try {
      ...
      localStorage.setItem("token", response.data); // Store JWT from NestJS
  ...
  // const handleSubmit = async (e: React.FormEvent) => {
    // e.preventDefault();
    // Standard email/password logic would go here if implemented on NestJS
  // };
...
{/* 
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            <div className="space-y-4">
              ...
              <div className="relative group">
                ...
                <button
                  ...
                  className="absolute cursor-pointer right-3.5 top-1/2 -translate-y-1/2 text-slate-400"
                >
            ...
            <Button
              type="submit"
              disabled={isLoading}
              className="w-full h-12 rounded-xl bg-emerald-600 hover:bg-emerald-700 text-white font-bold transition-all shadow-[0_4px_12px_rgba(16,185,129,0.25)] flex items-center justify-center gap-2"
            >
